...
# Convert Tur SVGs to React components
# Same process as Sport & Casino

SVG_DIR="src/SvsUiNova/brands/tur/svg-exports"
OUTPUT_BASE="src/SvsUiNova/brands/tur"

echo "🔄 Converting Tur SVGs to React components..."
echo ""
...
/**
 * ${component_name} - Tur ${component_type}
 *
...
for svg in "$SVG_DIR"/*.svg; do
  ...
  if [[ "$filename" == *"-icon"* ]]; then
    continue # Skip icons, will process later
  elif [[ "$filename" == *"cube-"* ]]; then
    continue # Skip cubes, will process later
  else
...
for svg in "$SVG_DIR"/*-icon.svg; do
  [ -e "$svg" ] || continue
...
for svg in "$SVG_DIR"/cube-*.svg; do
  [ -e "$svg" ] || continue
